        ...
        try {
            ...
            html += `
                <div class="mt-4 text-center">
                    <button id="specials-btn" class="btn btn-warning btn-lg">🌟Випадкова категорія</button>
                </div>
            `;
    ...
    async function loadCategoryItems(shortname) {
        try {
            ...
            data.items.forEach(item => {
                const placeholderUrl = `https://placehold.co/200x200/e9ecef/495057?text=${encodeURIComponent(item.shortname)}`;
                
                html += `
                    <div class="col-md-6 col-lg-3 mb-4">
                        <div class="card h-100 shadow-sm">
                            <img src="${placeholderUrl}" class="card-img-top p-2" alt="${item.name}" style="height: 200px; object-fit: contain;">
